...
#define MILLISECONDS_TO_SHOW_RED_FOR_KILLED_PROCESS 100
#define GET_OWN_ID
...
void ProcessModelPrivate::processChanged(KSysGuard::Process *process, bool onlyTotalCpu)
{
    ...
    if (onlyTotalCpu) {
        ...
        if (process->changes() & KSysGuard::Process::Memory) {
            totalUpdated++;
            QModelIndex index = q->createIndex(row, ProcessModel::HeadingVmSize, process);
            Q_EMIT q->dataChanged(index, index);
        }
        if (process->changes() & (KSysGuard::Process::Memory | KSysGuard::Process::MemoryPrecise)) {
            totalUpdated += 2;
